...
local config={
	...
	modules={
		Movement={
			Fly={enabled=false,speed=60,smooth=true},
			Noclip={enabled=false},
			HighJump={enabled=false,multiplier=2},
			Speed={enabled=false,walkspeed=32},
			InfiniteJump={enabled=false},
			Spider={enabled=false,climbSpeed=16},
			AirWalk={enabled=false,holdHeight=0}
		},
		Visual={
			CameraZoom={enabled=false,fov=70,min=20,max=120},
			RainbowTrails={enabled=false,hueSpeed=60}
		},
		Utility={
		...
		ESP={Enabled=true,Box=true,Tracer=true,HealthBar=true},
		Aimbot={Enabled=false,FOV=60,Smoothness=10},
		Invis={Enabled=false,FakeTransparency=0.5,SeatPosition=Vector3.new(0,0,0)},
		FakeMovement={MaxSpeed=20,Acceleration=40,Gravity=196,JumpPower=60}
	}
}
...
local function loadConfig()
	...
	if sv and sv.Value~="" then
		local ok,decoded=pcall(function() return HttpService:JSONDecode(sv.Value) end)
...
if config.modules.Utility.AntiAFK then
	spawn(function()
		while true do wait(60) if player and player:IsDescendantOf(game) then mouse.Icon=mouse.Icon end end
	end)
end
local input={forward=false,back=false,left=false,right=false,up=false,down=false,jump=false,click=false}
UserInputService.InputBegan:Connect(function(i,gp)
	if gp or not bindingsEnabled then return end
	if i.KeyCode==Enum.KeyCode.W then input.forward=true end
	if i.KeyCode==Enum.KeyCode.S then input.back=true end
	if i.KeyCode==Enum.KeyCode.A then input.left=true end
	if i.KeyCode==Enum.KeyCode.D then input.right=true end
	if i.KeyCode==Enum.KeyCode.E then input.up=true end
	if i.KeyCode==Enum.KeyCode.Q then input.down=true end
	if i.KeyCode==Enum.KeyCode.Space then input.jump=true end
	if i.UserInputType==Enum.UserInputType.MouseButton1 then input.click=true end
	...
	elseif config.keybinds.toggleSpeed and i.KeyCode==config.keybinds.toggleSpeed then
		...
		if config.modules.Movement.Speed.enabled then humanoid.WalkSpeed=config.modules.Movement.Speed.walkspeed else humanoid.WalkSpeed=16 end
	...
	elseif config.keybinds.toggleAirWalk and i.KeyCode==config.keybinds.toggleAirWalk then
		...
		if config.modules.Movement.AirWalk.enabled then config.modules.Movement.AirWalk.holdHeight=hrp.Position.Y end
	...
	elseif config.keybinds.toggleTrails and i.KeyCode==config.keybinds.toggleTrails then
		...
		if config.modules.Visual.RainbowTrails.enabled then enableRainbowTrails() else disableRainbowTrails() end
		...
		local function tryActivateNearestPrompt()
			...
			for _,prompt in pairs(Workspace:GetDescendants())do
				if prompt:IsA("ProximityPrompt") then
					...
					local promptPos
					if parentPart and parentPart:IsA("BasePart") then promptPos=parentPart.Position
					elseif prompt.Parent and prompt.Parent:IsA("Model") and prompt.Parent.PrimaryPart then promptPos=prompt.Parent.PrimaryPart.Position
					else promptPos=prompt.Parent.Position or nil end
					if promptPos then
						local d=(promptPos-pos).Magnitude
						if d<closestDist then closestDist=d closestPrompt=prompt end
					end
				end
			end
			if not closestPrompt then return end
			local promptParent=closestPrompt.Parent
			pcall(function() if closestPrompt.Trigger then closestPrompt:Trigger() end end)
			pcall(function() if closestPrompt.InputHoldBegin then closestPrompt:InputHoldBegin() end end)
			local be=promptParent:FindFirstChild("PromptActivated")
			if be and be:IsA("BindableEvent") then pcall(function() be:Fire(player) end) end
...
UserInputService.InputEnded:Connect(function(i,gp)
	...
	if i.KeyCode==Enum.KeyCode.W then input.forward=false end
	if i.KeyCode==Enum.KeyCode.S then input.back=false end
	if i.KeyCode==Enum.KeyCode.A then input.left=false end
	if i.KeyCode==Enum.KeyCode.D then input.right=false end
	if i.KeyCode==Enum.KeyCode.E then input.up=false end
	if i.KeyCode==Enum.KeyCode.Q then input.down=false end
	if i.KeyCode==Enum.KeyCode.Space then input.jump=false end
	if i.UserInputType==Enum.UserInputType.MouseButton1 then input.click=false end
end)
...
local function updateFly(dt, root)
	if not config.modules.Movement.Fly.enabled then return end
	local speed=config.modules.Movement.Fly.speed or 50
	local dir=Vector3.new(0,0,0)
	local cam=workspace.CurrentCamera
	local look=cam.CFrame.LookVector
	local right=cam.CFrame.RightVector
	if input.forward then dir=dir+Vector3.new(look.X,0,look.Z) end
	if input.back then dir=dir-Vector3.new(look.X,0,look.Z) end
	if input.right then dir=dir+Vector3.new(right.X,0,right.Z) end
	if input.left then dir=dir-Vector3.new(right.X,0,right.Z) end
	if input.up then dir=dir+Vector3.new(0,1,0) end
	if input.down then dir=dir-Vector3.new(0,1,0) end
	if dir.Magnitude>0 then dir=dir.Unit end
	local targetVel=dir*speed
	if config.modules.Movement.Fly.smooth then flyVelocity=flyVelocity:Lerp(targetVel,math.min(1,dt*8)) else flyVelocity=targetVel end
...
local function enableAirWalk(root)
	if airBody then airBody:Destroy() end
	airBody=Instance.new("BodyPosition")
	airBody.MaxForce=Vector3.new(1e5,1e5,1e5)
	airBody.P=1e4
	airBody.Position=Vector3.new(root.Position.X,config.modules.Movement.AirWalk.holdHeight or root.Position.Y,root.Position.Z)
	airBody.Parent=root
end
local function disableAirWalk()
	if airBody then airBody:Destroy() airBody=nil end
end
local lastTouched=nil
hrp.Touched:Connect(function(part)
	if part and part:IsA("BasePart")then
		lastTouched=part
		delay(0.2,function() lastTouched=nil end)
	end
end)
local function handleSpider(dt,root)
	if not config.modules.Movement.Spider.enabled then return end
	if lastTouched then
...
local function handleSpeed(activeHumanoid)
	if config.modules.Movement.Speed.enabled then
		if activeHumanoid then activeHumanoid.WalkSpeed=config.modules.Movement.Speed.walkspeed end
	else
		if activeHumanoid then activeHumanoid.WalkSpeed=16 end
	end
end
...
function enableRainbowTrails()
	for _,t in pairs(trailParts)do if t and t.Parent then t:Destroy() end end
	trailParts={}
...
function disableRainbowTrails()
	for _,t in pairs(trailParts)do if t then t:Destroy() end end
	trailParts={}
end
local function updateRainbow(dt)
	if not config.modules.Visual.RainbowTrails.enabled then return end
	...
	for _,tr in pairs(trailParts)do
		if tr and tr:IsA("Trail")then
			tr.Color=ColorSequence.new(Color3.fromHSV(hue/360,1,1),Color3.fromHSV(((hue+60)%360)/360,1,1))
		end
	end
end
local espBoxes={}
local function clearESP()
	for _,gui in pairs(espBoxes)do if gui and gui.Parent then gui:Destroy() end end
	espBoxes={}
end
local function updateESP()
	if not config.modules.ESP.Enabled then clearESP() return end
	local cam=workspace.CurrentCamera
	for _,plr in pairs(Players:GetPlayers())do
		if plr~=player and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")then
			...
			if onScreen then
				...
				if espBoxes[plr] and espBoxes[plr].Parent then espBoxes[plr]:Destroy() espBoxes[plr]=nil end
			end
		end
	end
end
...
local function updateAimbot(dt)
	if not config.modules.Aimbot.Enabled then return end
...
local function anchorRealParts(val)
	...
	for _,p in pairs(character:GetDescendants())do
		if p:IsA("BasePart")then
			if val then
				realAnchoredParts[p] = {Anchored = p.Anchored, CanCollide = p.CanCollide}
				p.Anchored = true
				p.CanCollide = false
			else
...
local function cloneToolsToFake()
	if not fakeModel then return end
	for _,child in pairs(fakeModel:GetChildren())do if child.Name=="__ToolClone" then child:Destroy() end end
...
local function tryActivateTool()
	...
	if realTool then
		pcall(function() realTool:Activate() end)
	end
	local cloneTool=fakeModel and fakeModel:FindFirstChild("__ToolClone")
	if cloneTool then
		pcall(function() cloneTool:Activate() end)
	end
end
local function enableInvis()
	if fakeModel then return end
	isInvisActive=true
...
local function disableInvis()
	...
	if fakeModel.PrimaryPart then
		pcall(function() hrp.CFrame = CFrame.new(fakeModel.PrimaryPart.Position + Vector3.new(0,3,0)) end)
	end
	if prevCameraSubject then workspace.CurrentCamera.CameraSubject=prevCameraSubject prevCameraSubject=nil end
	if fakeModel and fakeModel.Parent then fakeModel:Destroy() fakeModel=nil end
...
local function disableAll()
	for _,m in pairs(config.modules.Movement)do if type(m)=="table" and m.enabled~=nil then m.enabled=false end end
	...
	disableInvis()
	for k,_ in pairs(config.keybinds)do config.keybinds[k]=nil end
...
minimizeBtn.MouseButton1Click:Connect(function()
	...
	for _,c in pairs(frame:GetChildren())do
		if (c:IsA("TextButton") or c:IsA("TextLabel")) and c~=header and c~=minimizeBtn and c~=closeBtn and c~=saveBtn then
...
local function addToggleLabel(text,getter,setter)
	...
	btn.MouseButton1Click:Connect(function()
		setter(not getter())
		lbl.Text=tostring(getter())
		if text=="Air Walk" and getter() then config.modules.Movement.AirWalk.holdHeight=hrp.Position.Y end
		if text=="High Jump" then setHighJump(getter()) end
		if text=="Rainbow Trails" then if getter() then enableRainbowTrails() else disableRainbowTrails() end end
		if text=="Invisibility" and getter() then enableInvis() else disableInvis() end
	end)
	y=y+30
end
addToggleLabel("Fly",function() return config.modules.Movement.Fly.enabled end,function(v) config.modules.Movement.Fly.enabled=v end)
addToggleLabel("Noclip",function() return config.modules.Movement.Noclip.enabled end,function(v) config.modules.Movement.Noclip.enabled=v end)
addToggleLabel("Infinite Jump",function() return config.modules.Movement.InfiniteJump.enabled end,function(v) config.modules.Movement.InfiniteJump.enabled=v end)
addToggleLabel("Air Walk",function() return config.modules.Movement.AirWalk.enabled end,function(v) config.modules.Movement.AirWalk.enabled=v end)
addToggleLabel("Speed",function() return config.modules.Movement.Speed.enabled end,function(v) config.modules.Movement.Speed.enabled=v end)
addToggleLabel("Spider",function() return config.modules.Movement.Spider.enabled end,function(v) config.modules.Movement.Spider.enabled=v end)
addToggleLabel("High Jump",function() return config.modules.Movement.HighJump.enabled end,function(v) config.modules.Movement.HighJump.enabled=v end)
addToggleLabel("Rainbow Trails",function() return config.modules.Visual.RainbowTrails.enabled end,function(v) config.modules.Visual.RainbowTrails.enabled=v end)
addToggleLabel("ESP",function() return config.modules.ESP.Enabled end,function(v) config.modules.ESP.Enabled=v end)
addToggleLabel("Aim Assist",function() return config.modules.Aimbot.Enabled end,function(v) config.modules.Aimbot.Enabled=v end)
addToggleLabel("Invisibility",function() return config.modules.Invis.Enabled end,function(v) config.modules.Invis.Enabled=v end)
...
local function refreshTeleportList()
	for _,c in pairs(frame:GetChildren())do if c.Name=="TPBtn" then c:Destroy() end end
	local yy=y
	for _,plr in pairs(Players:GetPlayers())do
		if plr~=player then
			...
			b.MouseButton1Click:Connect(function()
				if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")then teleportTo(plr.Character.HumanoidRootPart.Position) end
			end)
			yy=yy+22
		end
	end
...
Players.PlayerRemoving:Connect(function() refreshTeleportList() end)
refreshTeleportList()
...
UserInputService.InputBegan:Connect(function(inp,gp)
	if gp then return end
	if inp.KeyCode==config.keybinds.toggleGui then tryToggleGUI() end
end)
local function updateFakeMovement(dt)
	...
	if config.modules.Movement.Fly.enabled then
		...
		if dir.Magnitude>0 then dir=dir.Unit targetVel=Vector3.new(dir.X*maxSpeed,0,dir.Z*maxSpeed) end
		fakeVelocity=fakeVelocity:Lerp(Vector3.new(targetVel.X,0,targetVel.Z),math.clamp(accel*dt/ maxSpeed,0,1))
		if input.jump then fakeVerticalVel=math.max(fakeVerticalVel, jumpPower) end
	...
	cloneToolsToFake()
	if input.click then tryActivateTool() end
end
RunService.RenderStepped:Connect(function(dt)
	if not character or not humanoid or not hrp then return end
	if not isInvisActive then
		if config.modules.Movement.Fly.enabled then humanoid.PlatformStand=true updateFly(dt, hrp) else humanoid.PlatformStand=false end
		...
		if config.modules.Movement.AirWalk.enabled then
			if not airBody then enableAirWalk(hrp) end
			airBody.Position=Vector3.new(hrp.Position.X,config.modules.Movement.AirWalk.holdHeight or hrp.Position.Y,hrp.Position.Z)
		else disableAirWalk() end
		handleSpider(dt, hrp)
		handleSpeed(humanoid)
		applyCameraZoom()
		if config.modules.Movement.HighJump.enabled then setHighJump(true) end
	else
		if config.modules.Movement.Noclip.enabled then setModelCanCollide(fakeModel, false) else setModelCanCollide(fakeModel, true) end
		if config.modules.Movement.AirWalk.enabled then
			if not airBody then enableAirWalk(fakeHRP) end
			airBody.Position=Vector3.new(fakeHRP.Position.X,config.modules.Movement.AirWalk.holdHeight or fakeHRP.Position.Y,fakeHRP.Position.Z)
		else disableAirWalk() end
